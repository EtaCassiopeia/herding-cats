...
package object domain {
  ...
  case class GetById(id: String)
      extends AppInstruction[Option[Person]]
      with DatabaseInstruction[Option[Person]]
  case class Save(person: Person)
      extends AppInstruction[Unit]
      with DatabaseInstruction[Unit]
  case class Log(info: String)
      extends AppInstruction[Unit]
      with LogInstruction[Unit]

  // Smart constructors
  def getById(id: String): Free[AppInstruction, Option[Person]] =
    GetById(id).lift //Free.liftF(GetById(id))
