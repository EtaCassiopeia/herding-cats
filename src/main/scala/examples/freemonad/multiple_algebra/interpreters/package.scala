...
import examples.freemonad.multiple_algebra.domain._
...
package object interpreters {
  ...
  class InMemoryDbInterpreter extends DatabaseInterpreter {
    ...
    override def run[A](dbInstruction: DatabaseInstruction[A]): Task[A] =
      dbInstruction match {
        case GetById(id) =>
          Task(map.get(id))
  ...
  class AppInstructionInterpreter(logInterpreter: LogInterpreter,
                                  dbInterpreter: DatabaseInterpreter) {
